...
export abstract class ArrayUtils {
     ...
     * @returns
     */
    public static treeExpand<T extends IIdentityGroup<T>>(
        array: TGroupCollection<T>,
    ): T[] {
        const list: T[] = [];
     ...
     * @returns
     */
    public static treeTraverseFilter<T extends IIdentityGroup<T>>(
        item: T,
        selects: string[],
        del: boolean,
     ...
     * @returns
     */
    public static treeTraverseFind<T extends IIdentityGroup<T>>(
        array: TGroupCollection<T>,
        filter: (
            value: T,
            index: number,
        ...
        for (let index = 0; index < list.length; index++) {
            ...
            if (item.children) {
                const result: T | undefined = this.treeTraverseFind(
                    item.children,
                    filter,
                );
     ...
     * @returns
     */
    public static treeTraverseFindAll<T extends IIdentityGroup<T>>(
        array: TGroupCollection<T>,
        filter: (
            value: T,
            index: number,
